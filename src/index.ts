...
const ExtractFeedContentSchema = z.object({
  ...
  format: z.enum(['markdown', 'text', 'html', 'json']).optional().default('text')
    ...
    const formatItem = (item: FeedItem): string | object => {
      ...
      switch (args.format) {
        ...
        case 'html':
          ...
          if (metadataText) htmlParts.push(`<div class="metadata">${metadataText.replace(/\n/g, '<br>')}</div>`);
...
const GetFeedHeadlinesSchema = z.object({
  ...
  format: z.enum(['markdown', 'text', 'html', 'json']).optional().default('json')
    .describe('Output format for the headlines'),
});

server.addTool({
  name: 'get_feed_headlines',
  description: 'Gets a list of headlines from a feed, including title, summary, and URL.',
